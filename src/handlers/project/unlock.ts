...
export default async function unlockProject(req: FastifyRequest, res: FastifyReply) {
    ...
    try {
        ...
        console.log(`Error unlocking project: ${error}`)
        return res.status(500).send({ error: 'Failed to unlock project' })
    }
}
